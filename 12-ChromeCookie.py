...
def build_opener_with_chrome_cookies(domain=None):
    ...
    for row in coon.execute(sql):
        ...
        print(cookie_item,cookie_item.domain)
...
html_doc = opener.open('https://www.zhihu.com/people/jueee/answers').read()

print(html_doc)
content = re.search(r'<a class="question_link" href="(.*?)">', html_doc)
print(content)

for match in re.finditer(r'<a class="question_link" href="(.*?)">\s*([^\s]*)\s*</a>', html_doc):
    link = match.group(1)
    title = match.group(2)
    print(link,title)